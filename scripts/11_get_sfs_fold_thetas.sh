...
# run as sbatch 11_get_sfs_fold_thetas.sh nlf_all_samples 50
...
do
	...
	pop=$(sed -n ${x}p $infile)
		...
		echo "realSFS results_sfs/${pop}_${thresh}.saf.idx -fold 1 > results_sfs_folded/${pop}_${thresh}.folded.sfs" >> ${pop}_folded.sh
		# bootstrap
		echo "realSFS results_sfs/${pop}_${thresh}.saf.idx -bootstrap 100 -P 10 > results_sfs_folded/${pop}_${thresh}_b.folded.sfs" >> ${pop}_folded.sh

		# make a plot
		echo "~/scripts/plotSFS.R results_sfs_folded/${pop}_${thresh}.folded.sfs" >> ${pop}_folded.sh

		# calc thetas for each site:
		mkdir -p results_thetas

		# this makes .thetas.idx
		echo "realSFS saf2theta results_sfs/${pop}_${thresh}.saf.idx -sfs results_sfs_folded/${pop}_${thresh}.folded.sfs -outname results_thetas/${pop}_${thresh}_folded" >> ${pop}_folded.sh
		...
		echo "thetaStat do_stat results_thetas/${pop}_${thresh}_folded.thetas.idx -win 1 -step 1 -outnames results_thetas/${pop}_${thresh}_folded_gw_thetasWin.gz" >> ${pop}_folded.sh
